  ...
  return(
    <span style={{display:"inline-flex",alignItems:"center",gap:2}}>
      <svg width={w} height={h+2} style={{overflow:"visible",verticalAlign:"middle"}}>
        ...
        <circle cx={(nums.length-1)/(nums.length-1)*w} cy={h-((nums[nums.length-1]-mn)/rng)*h} r="2.5" fill={color}/>
      </svg>
...
export default function App(){
  ...
  const saveVitals=async()=>{
    ...
    await fetch(`${PROXY}/api/notion/patients/${pt.notionId}`, {
      ...
      body:JSON.stringify({
        ...
        SpO2: { rich_text:[{text:{content:vForm.spo2||pt.spo2||""}}] },
  ...
  const assignMonitor=()=>{
    ...
    if(!pt)return;
    const newProtos=[...new Set([...(pt.protocols||[]),...mForm.protocols])];
...
Critical patients: ${JSON.stringify(critical.map(p=>({name:p.name,bed:p.bed,diagnosis:p.diagnosis,drips:p.drips,bp:p.bp,spo2:p.spo2,pulse:p.pulse,gcs:p.gcs,uop:p.uop,notes:p.notes,recent:p.vitalsHistory?.slice(-3)})))}.
...
Pending tasks: ${JSON.stringify(pending.map(t=>({text:t.text,patient:t.patientName,assigned:t.assignedTo,due:t.dueTime,status:t.status})))}.
Be concise and clinically precise. Include MAP calculations, vasopressor endpoints, specific numbers.`;
      ...
      setAiMsgs(m=>[...m,{role:"assistant",text:txt||"Try again."}]);
    ...
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:"#f0f4f8",minHeight:"100vh",maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{background:"#0c1526",padding:"11px 15px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
        <div>
          ...
          <div style={{color:"#475569",fontSize:10,fontFamily:"'DM Mono',monospace",marginTop:1}}>CSM Hospital · Kalwa · Unit 2 · {new Date().toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</div>
        </div>
      ...
      <div style={{background:"#0f172a",padding:"10px 16px",display:"flex"}}>
        {[{l:"Admitted",v:patients.length,c:"#60a5fa"},{l:"Critical",v:critical.length,c:"#f87171"},{l:"On drips",v:patients.filter(p=>p.drips?.length>0).length,c:"#fb923c"},{l:"Tasks",v:pending.length,c:"#fbbf24"}].map((s,i)=>(
      ...
      {!loading&&critical.length>0&&!sel&&(
        <div style={{background:"#1a0505",borderBottom:"1px solid #450a0a",padding:"8px 11px"}}>
          ...
          {critical.map(pt=>(
            <div key={pt.id} onClick={()=>{setSel(pt.id);setView("patients");setDtab("vitals");}} style={{background:"#2d0707",borderRadius:8,padding:"8px 10px",marginBottom:4,cursor:"pointer",border:"1px solid #7f1d1d"}}>
      ...
      <div style={{padding:"10px 10px 100px"}}>
          ...
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={()=>setModal("monitor")} style={{...SMBTN,background:"#fff7ed",color:"#9a3412",border:"1px solid #fed7aa"}}>+ Protocol</button>
              ...
              {patients.map(pt=>{
                ...
                return(
                  <div key={pt.id} onClick={()=>{setSel(pt.id);setView("patients");setDtab("vitals");}} style={{background:"#fff",borderRadius:12,padding:11,border:`1.5px solid ${pt.status==="critical"?"#fca5a5":"#e2e8f0"}`,cursor:"pointer"}}>
                    ...
                    {pt.drips?.length>0&&<div style={{fontSize:10,color:"#ea580c",marginBottom:4}}>💉 {pt.drips.length} drip{pt.drips.length>1?"s":""}</div>}
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:10,color:"#475569",fontFamily:"'DM Mono',monospace"}}>SpO₂ {pt.spo2}% · HR {pt.pulse}</span>
                      {bpH.length>1&&<Sparkline data={bpH} color={pt.status==="critical"?"#ef4444":"#3b82f6"} h={20} w={44}/>}
                    </div>
                    {ptT.length>0&&<div style={{marginTop:5,background:"#fef9c3",borderRadius:5,padding:"2px 6px",fontSize:10,color:"#854d0e"}}>{ptT.filter(t=>t.status==="overdue").length>0?"⚠":"•"} {ptT.length} task{ptT.length>1?"s":""} pending</div>}
                  </div>
                );
              })}
            </div>
        ...
        {!loading&&view==="patients"&&!sel&&(
          <div>
            ...
            {patients.map(pt=>{
              const sc=SC[pt.status]||SC.stable;
              return(
                <div key={pt.id} onClick={()=>{setSel(pt.id);setDtab("vitals");}} style={{background:"#fff",borderRadius:11,padding:"11px 13px",marginBottom:7,border:"1px solid #e2e8f0",cursor:"pointer",display:"flex",alignItems:"center",gap:11}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {pt.drips?.length>0&&<div style={{fontSize:10,color:"#ea580c",marginTop:2}}>💉 {pt.drips.join(" · ")}</div>}
                  </div>
                ...
                <div>
                  ...
                  {selPt.drips?.length>0&&(
                    <div style={{marginTop:6,display:"flex",flexDirection:"column",gap:3}}>
                      {selPt.drips.map((d,i)=><span key={i} style={{fontSize:10,color:"#fb923c",background:"#431407",padding:"2px 8px",borderRadius:5,width:"fit-content"}}>💉 {d}</span>)}
                    </div>
                  )}
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'DM Mono',monospace",fontWeight:700,fontSize:15,color:"#60a5fa"}}>{selPt.bed}</div>
                  <div style={{color:"#475569",fontSize:10,marginTop:2}}>{selPt.admitDate}</div>
                  <div style={{color:"#475569",fontSize:10}}>{selPt.attending}</div>
                </div>
              </div>
              <div style={{marginTop:9,display:"flex",gap:5,flexWrap:"wrap"}}>
                {["stable","review","critical","discharged"].map(s=>{const sc=SC[s];return(
            ...
            {dtab==="vitals"&&(
              <div>
                {selPt.protocols?.length>0&&(
                  <div style={{background:"#fff7ed",borderRadius:10,padding:"9px 12px",marginBottom:8,border:"1px solid #fed7aa"}}>
                    <div style={{fontSize:10,fontWeight:700,color:"#9a3412",textTransform:"uppercase",marginBottom:5}}>Active Monitoring Protocols</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                      {selPt.protocols.map(pid=>{
                        const proto=MONITORING_PROTOCOLS.find(p=>p.id===pid);
                        if(!proto)return null;
                        return <span key={pid} style={{fontSize:11,color:proto.color,background:"#fff",border:`1px solid ${proto.color}22`,padding:"2px 8px",borderRadius:12}}>{proto.icon} {proto.label}</span>;
                      })}
                    </div>
                  </div>
                )}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
                    {[
                      ...
                      <div key={l} style={{background:w&&v!=="—"?"#fff1f2":"#f8fafc",borderRadius:8,padding:"8px 9px",border:`1px solid ${w&&v!=="—"?"#fecdd3":"#e2e8f0"}`}}>
                        ...
                        {h&&h.length>1&&<div style={{marginTop:3}}><Sparkline data={h} color={w?"#ef4444":"#3b82f6"} h={18} w={50}/></div>}
                      </div>
                    ))}
                  </div>
                </Sec>
                    ...
                    <div style={{overflowX:"auto"}}>
                      <table style={{width:"100%",fontSize:11,borderCollapse:"collapse",fontFamily:"'DM Mono',monospace"}}>
                        <thead><tr style={{background:"#f8fafc"}}>
                          {["Time","BP","SpO₂","HR","Temp","RR","UOP","RBS","GCS"].map(h=>(
                        ...
                        <tbody>
                          {[...selPt.vitalsHistory].reverse().map((v,i)=>(
                            <tr key={i} style={{borderBottom:"1px solid #f1f5f9"}}>
                              {[v.time,v.bp,v.spo2?`${v.spo2}%`:"—",v.pulse,v.temp,v.rr,v.uop||"—",v.rbs||"—",v.gcs||"—"].map((c,j)=>(
                                <td key={j} style={{padding:"4px 5px",color:j===0?"#64748b":"#0f172a",whiteSpace:"nowrap"}}>{c}</td>
                              ))}
                            </tr>
                          ))}
                        </tbody>
                ...
                <Sec title="Medications">
                  {selPt.meds.length===0?<div style={{color:"#cbd5e1",fontSize:13}}>None.</div>:selPt.meds.map((m,i)=>(
                    <div key={i} style={{fontSize:12,color:"#1e293b",padding:"5px 0",borderBottom:i<selPt.meds.length-1?"1px solid #f1f5f9":"none",lineHeight:1.5}}>
                      <span style={{color:"#94a3b8",fontFamily:"'DM Mono',monospace",marginRight:5,fontSize:10}}>Rx</span>{m}
                    </div>
                  ))}
                </Sec>
              </div>
            ...
            {dtab==="labs"&&(
              <div>
                {selPt.labHistory?.length>0&&(
                  <Sec title="Lab Timeline">
                    {selPt.labHistory.map((l,i)=>{
                      const fc=l.flag==="critical"?{bg:"#450a0a",tx:"#f87171",bd:"#7f1d1d"}:l.flag==="high"?{bg:"#451a03",tx:"#fb923c",bd:"#92400e"}:{bg:"#f0fdf4",tx:"#166534",bd:"#bbf7d0"};
                      return(
                        <div key={i} style={{display:"flex",gap:10,marginBottom:9,alignItems:"flex-start"}}>
                          <div style={{display:"flex",flexDirection:"column",alignItems:"center",paddingTop:4}}>
                            <div style={{width:6,height:6,borderRadius:"50%",background:l.flag==="critical"?"#ef4444":l.flag==="high"?"#f97316":"#10b981"}}/>
                            {i<selPt.labHistory.length-1&&<div style={{width:1,flex:1,background:"#e2e8f0",marginTop:2}}/>}
                          </div>
                          <div style={{flex:1,paddingBottom:i<selPt.labHistory.length-1?6:0}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2}}>
                              <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#64748b"}}>{l.time}</span>
                              <span style={{background:fc.bg,color:fc.tx,fontSize:9,padding:"1px 7px",borderRadius:10,fontWeight:700,border:`1px solid ${fc.bd}`}}>{(l.flag||"").toUpperCase()}</span>
                            </div>
                            <div style={{fontSize:13,fontWeight:600,color:"#0f172a"}}>{l.label}</div>
                            <div style={{fontSize:12,color:"#475569",fontFamily:"'DM Mono',monospace"}}>{l.value}</div>
                          </div>
                        </div>
                      );
                    })}
                ...
                <Sec title="All Investigations">
                  {selPt.labs.length===0?<div style={{color:"#cbd5e1",fontSize:13}}>None.</div>:selPt.labs.map((l,i)=>(
                    <div key={i} style={{fontSize:12,color:"#334155",padding:"5px 0",borderBottom:i<selPt.labs.length-1?"1px solid #f8fafc":"none",lineHeight:1.5}}>
                      <span style={{color:"#94a3b8",marginRight:6}}>◦</span>{l}
                    </div>
                  ))}
                </Sec>
              </div>
            ...
            {dtab==="tasks"&&(
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>{setMForm(f=>({...f,patientId:sel}));setModal("monitor");}} style={{...SMBTN,background:"#fff7ed",color:"#9a3412",border:"1px solid #fed7aa"}}>+ Protocol</button>
                    <button onClick={()=>{setTForm(f=>({...f,patientId:sel}));setModal("task");}} style={{...SMBTN,background:"#3b82f6",color:"#fff",border:"none"}}>+ Task</button>
                  </div>
                </div>
            ...
            {TEAM.map(member=>{
              ...
              return(
                <div key={member} style={{marginBottom:14}}>
                  <div style={{fontSize:11,fontWeight:700,background:hasOverdue?"#450a0a":"#eff6ff",color:hasOverdue?"#fca5a5":"#1e40af",border:`1px solid ${hasOverdue?"#7f1d1d":"#bfdbfe"}`,borderRadius:7,padding:"4px 10px",marginBottom:6,display:"inline-flex",alignItems:"center",gap:6}}>
                    {hasOverdue&&<span>⚠</span>}{member} <span style={{opacity:0.6}}>({mt.length})</span>
                  </div>
                  {mt.map(t=><TaskCard key={t.id} task={t} onChange={updTaskStatus} showPt/>)}
                </div>
              );
            })}
        ...
        {view==="labs"&&(
          <div>
            ...
            {critical.filter(p=>p.labHistory?.some(l=>l.flag==="critical")).map(pt=>(
              <div key={pt.id} style={{background:"#1a0505",borderRadius:10,padding:"10px 12px",marginBottom:8,border:"1px solid #450a0a"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                  <span style={{fontWeight:700,fontSize:13,color:"#fca5a5"}}>{pt.name}</span>
                  <span style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"#64748b"}}>{pt.bed}</span>
                </div>
                {pt.labHistory?.filter(l=>l.flag==="critical").map((l,i)=>(
                  <div key={i} style={{fontSize:12,color:"#f87171",marginBottom:2}}>⚠ [{l.time}] {l.label}: {l.value}</div>
                ))}
            ...
            {patients.map(pt=>(
              <div key={pt.id} style={{background:"#fff",borderRadius:10,padding:"11px 13px",marginBottom:8,border:"1px solid #e2e8f0"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
                  <div><span style={{fontWeight:600,fontSize:14,color:"#0f172a"}}>{pt.name}</span><span style={{fontSize:12,color:"#94a3b8",marginLeft:8,fontFamily:"'DM Mono',monospace"}}>{pt.bed}</span></div>
                  <button onClick={()=>{setSel(pt.id);setModal("lab");}} style={{fontSize:11,color:"#3b82f6",background:"#eff6ff",border:"1px solid #bfdbfe",padding:"3px 8px",borderRadius:6,cursor:"pointer",fontWeight:600}}>+ Add</button>
                </div>
                {pt.labs.slice(0,4).map((l,i)=>(
                  <div key={i} style={{fontSize:12,color:"#334155",padding:"3px 0",borderBottom:i<Math.min(pt.labs.length,4)-1?"1px solid #f8fafc":"none"}}><span style={{color:"#94a3b8",marginRight:5}}>◦</span>{l}</div>
                ))}
              </div>
            ))}
          </div>
      ...
      {modal==="vitals"&&(
        <Mdl title={`Record Vitals — ${selPt?.name}`} onClose={closeModal}>
          ...
          {selPt?.drips?.length>0&&<div style={{background:"#fff7ed",borderRadius:8,padding:"8px 10px",marginBottom:12,fontSize:12,color:"#9a3412"}}><strong>On drips:</strong> {selPt.drips.join(" · ")}</div>}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:14}}>
            ...
            <div>
              ...
              {MONITORING_PROTOCOLS.map(proto=>{
                ...
                return(
                  <div key={proto.id} onClick={()=>setMForm(f=>({...f,protocols:on?f.protocols.filter(x=>x!==proto.id):[...f.protocols,proto.id]}))} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",background:on?"#f8fafc":"#fff",border:`1.5px solid ${on?proto.color:"#e2e8f0"}`,borderRadius:9,marginBottom:5,cursor:"pointer"}}>
                    <div style={{width:18,height:18,borderRadius:5,background:on?proto.color:"#fff",border:`2px solid ${on?proto.color:"#cbd5e1"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      {on&&<span style={{color:"#fff",fontSize:11,lineHeight:1}}>✓</span>}
                    </div>
                    <div>
                      <div style={{fontSize:13,fontWeight:on?700:400,color:on?proto.color:"#0f172a"}}>{proto.icon} {proto.label}</div>
                      <div style={{fontSize:10,color:"#94a3b8"}}>Every {proto.interval} min · {proto.vitals.join(", ")}</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div>
              <Lbl c="Assign to"/>
              <select value={mForm.assignTo} onChange={e=>setMForm(f=>({...f,assignTo:e.target.value}))} style={INP}>
      ...
      {modal==="task"&&(
        <Mdl title="Assign Task to Resident" onClose={closeModal}>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div>
              <Lbl c="Patient"/>
              <select value={tForm.patientId} onChange={e=>setTForm(f=>({...f,patientId:e.target.value}))} style={INP}>
                <option value="">Select patient</option>
                {patients.map(p=><option key={p.id} value={p.id}>{p.name} ({p.bed})</option>)}
              </select>
            </div>
            <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div>
                <Lbl c="Assign to"/>
                <select value={tForm.assignedTo} onChange={e=>setTForm(f=>({...f,assignedTo:e.target.value}))} style={INP}>
                  {TEAM.map(m=><option key={m}>{m}</option>)}
                </select>
              </div>
              <div>
                <Lbl c="Due time"/>
                <input type="time" value={tForm.dueTime} onChange={e=>setTForm(f=>({...f,dueTime:e.target.value}))} style={INP}/>
              </div>
            </div>
            <div>
              <Lbl c="Priority"/>
        ...
        <div style={{position:"fixed",inset:0,background:"#0c1526",zIndex:200,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"13px 15px",borderBottom:"1px solid #1e293b",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{color:"#475569",fontSize:11}}>Knows all patients · vasopressor protocols · clinical context</div>
            </div>
          ...
          <div style={{padding:"7px 12px 5px",display:"flex",flexWrap:"wrap",gap:5}}>
            {["Summarise ICU patients","Norad titration for MAP <65","Night handover note","Overdue tasks","Vasopressin dose range"].map(q=>(
...
function NotesTab({pt,onSave}){
  ...
  return(
    <Sec title="Doctor Notes" action={<button onClick={()=>{setTxt(pt.notes);setEd(true);}} style={EBTN}>Edit</button>}>
      {ed?(
        <div>
          <textarea value={txt} onChange={e=>setTxt(e.target.value)} rows={7} style={{...INP,resize:"vertical",fontFamily:"'DM Sans',sans-serif",lineHeight:1.7,marginBottom:8}} placeholder="SOAP note, clinical observations, plan…"/>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{onSave(txt);setEd(false);}} style={{...SBTN,flex:2}}>Save</button>
            <button onClick={()=>setEd(false)} style={{...CBTN,flex:1}}>Cancel</button>
          </div>
        </div>
      ):(
        <div style={{fontSize:13,color:"#334155",lineHeight:1.8,whiteSpace:"pre-wrap",minHeight:50}}>{pt.notes||<span style={{color:"#cbd5e1"}}>No notes yet.</span>}</div>
      )}
    </Sec>
  );
}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:3}}>
        <div style={{flex:1}}>
          <div style={{fontSize:13,color:"#0f172a",fontWeight:500,lineHeight:1.4}}>
            {task.isMonitoring&&<span style={{fontSize:9,color:"#7c3aed",background:"#ede9fe",borderRadius:4,padding:"1px 5px",marginRight:5,fontWeight:700}}>PROTO</span>}
            {task.text}
          </div>
          {task.notes&&<div style={{fontSize:11,color:"#64748b",marginTop:2,fontStyle:"italic"}}>→ {task.notes}</div>}
          {showPt&&<div style={{fontSize:11,color:"#64748b",marginTop:2}}>{task.bed} · {task.patientName}</div>}
          <div style={{fontSize:11,color:"#94a3b8",marginTop:2}}>{task.assignedTo}{task.dueTime?` · ⏱ ${task.dueTime}`:""}</div>
        </div>
        <select value={task.status} onChange={e=>onChange(task.id,e.target.value)} style={{fontSize:10,border:`1px solid ${tc.bd}`,background:tc.bg,color:tc.tx,fontWeight:700,borderRadius:6,padding:"3px 5px",cursor:"pointer",marginLeft:8,fontFamily:"'DM Sans',sans-serif"}}>
